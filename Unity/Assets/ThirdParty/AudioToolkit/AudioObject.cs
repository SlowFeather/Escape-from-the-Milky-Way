...
#endif
{
	...
	public void FadeIn( float fadeInTime )
	{
        ...
        _primaryFader.FadeIn( fadeInTime, audioObjectTime );
        _UpdateFadeVolume();
	}
    ...
    public void Pause( float fadeOutTime )
	{
        ...
        if ( fadeOutTime > 0 )
        {
            ...
            FadeOut( fadeOutTime );
            StartCoroutine( _WaitThenPause( fadeOutTime ) );
            return;
        }
        _PauseNow();
	}
    ...
    private IEnumerator _WaitThenPause( float waitTime )
    {
    ...
    private void _UpdateFadeVolume()
    {
        ...
        // primary AudioSource
        float fadeVolumePrimary = _primaryFader.Get( out finishedFadeOut );
        if ( finishedFadeOut )
        {
            if ( _stopRequested )
            {
        ...
        if ( _audioSource2 != null )
        {
            float fadeVolumeSecondary = _secondaryFader.Get( out finishedFadeOut );
