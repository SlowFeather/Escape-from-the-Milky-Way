...
public class ReferenceCollectorEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		if (GUILayout.Button("生成字段代码"))
		{
			StringBuilder build = new StringBuilder();

			foreach (var item in referenceCollector.data)
			{
				build.AppendLine($"public GameObject {item.key};");
			}
		...
		if (GUILayout.Button("生成获取代码"))
		{
			...
			build.AppendLine($"ReferenceCollector rc_{referenceCollector.gameObject.name} = this.GetParent<UI>().GameObject.GetComponent<ReferenceCollector>();");

			foreach (var item in referenceCollector.data)
			{
				build.AppendLine($"{item.key} = rc_{referenceCollector.gameObject.name}.Get<GameObject>(\"{item.key}\");");
			}
		...
		for (int i = referenceCollector.data.Count - 1; i >= 0; i--)
		{
			...
			if (GUILayout.Button("<-"))
			{
				ClipboardHelper.ClipBoard = $"GameObject {referenceCollector.data[i].key} = this.GetParent<UI>().GameObject.GetComponent<ReferenceCollector>().Get<GameObject>(\"{referenceCollector.data[i].key}\");";
			}
			if (GUILayout.Button("<"))
			{
				ClipboardHelper.ClipBoard = $"GameObject {referenceCollector.data[i].key} = rc.Get<GameObject>(\"{referenceCollector.data[i].key}\");";
			}
